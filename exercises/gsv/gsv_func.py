...
def create_sine_int(f0,T,phi):
	...
	nt = (int) (tmax/dt)            # number of time samples 
	time = np.arange(0,nt*dt,dt)    # define time vector
	sine = np.sin(w*time+phi)           # calculate sine function
...
def create_cos_int(f0,T,phi):
	...
	nt = (int) (tmax/dt)            # number of time samples 
	time = np.arange(0,nt*dt,dt)    # define time vector
	cos = np.cos(w*time+phi)           # calculate sine function
...
def comp_KKF_xy(lags,AKF,f0,phi,T,title,xlabel,ylabel,sw):
	...
	if(sw==2):
		KKF_an = 1./(2.*T*w0) * ((np.sin(2*w0*(T-np.abs(lags))+w0*lags+phi)/2.) - (np.sin(w0*lags+phi)/2.) + w0*(T-np.abs(lags)) * np.cos(w0*lags+phi))
	if(sw==3):
		KKF_an = 1./(2.*T*w0) * ((np.sin(2*w0*(T-np.abs(lags))+w0*lags+phi)/2.) - (np.sin(w0*lags+phi)/2.))        
	# KKF_an = 1./(2.*T*w0) * ((np.sin(2*w0*(T-np.abs(lags))+w0*lags+phi)/2.) - (np.sin(w0*lags+phi)/2.) + w0*(T-np.abs(lags)) * np.cos(w0*lags+phi))
	# KKF_an = 1./(2.*T*w0) * ((np.sin(2*w0*(T-np.abs(lags))+w0*lags+phi)/2.) - (np.sin(w0*lags+phi)/2.))
	...
	# plot AKF
	plt.plot(lags, AKF, 'b-',lw=3,label='numeric')
	if(sw==1):
