...
def slant_stack_intro(vstack,stackcorr):
    ...
    for it in range(nt):
        ts=(it-t0)*dt
        src[it]=-2*sigma*ts*np.exp(-(sigma*ts)**2)
    
    # Convolve source wavelet with Greens function
    # --------------------------------------------
    for i in range(0,nx):
        conv = np.convolve(src,TT[i,:])
        TT[i,:] = conv[1:nt+1]

    if(stackcorr==True):
        
        # Correct time-shift shot gather by tc = - offset / vstack 
        # --------------------------------------------------------
        TTc = np.zeros((nx,nt))  # initialize corrected shot gather
        tstack = (x / vstack)    # traveltime [s]
        nc = (tstack/dt)         # traveltime [sample#]
        nc = nc.astype(np.int)   # convert floats to integers
